const moment = require('moment'); // @TODO: refactor from moment to Luxon
...
class PrettyStream extends Transform {
  ...
  write(data, enc, cb) {
    // Bunyan sometimes passes things as objects. Because of this, we need
    // to make sure the data is converted to JSON
  ...
  /**
   * @param {Express.Request} data.req
   * @param {Express.Response} data.res
   */
  _transform(data, enc, cb) {
    ...
    try {
      ...
      // If data is not JSON we don't want to continue processing
      // as if it is
      return;
    }
    ...
    // CASE: bunyan passes each plain string/integer as `msg` attribute
    // (logging.info('Hey !')
    // CASE: bunyan extended this by figuring out a message in an error
    ...
    // CASE: log objects in pretty JSON format
      ...
      // can bec extended to define from outside, but not important
      try {
        ...
        if (isObject(value) && value.message && value.stack) {
          ...
          if (value.errorDetails) {
            error += colorize(
              ...
              'ERROR DETAILS:\n' +
              prettyjson.render(
                Array.isArray(value.errorDetails) ? value.errorDetails[0] : value.errorDetails,
                {noColor: true},
                4
              )
            ) + '\n\n';
